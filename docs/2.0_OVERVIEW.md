...
4. **Clean Separation** - Domain → Infrastructure → Application layers
...
┌─────────────────────────────────────────────────────────────┐
│                    CLI Scripts Layer                        │
│          (analyze.py, scan.py, backfill.py)                 │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
│                Application Layer (Business Logic)           │
│  - TickerAnalyzer (orchestrates metrics)                    │
│  - ImpliedMoveCalculator, VRPCalculator                     │
│  - ConsistencyAnalyzer, TermStructureAnalyzer               │
│  - SkewAnalyzer, ExecutionQualityAnalyzer                   │
└────────────────────────┬────────────────────────────────────┘
                         │
┌────────────────────────▼────────────────────────────────────┐
...
┌────────────────────────▼────────────────────────────────────┐
│                   Domain Layer (Types)                      │
│  - Types: Money, Percentage, OptionChain, ImpliedMove       │
│  - Enums: Recommendation, Quality, RiskLevel, Action        │
│  - Protocols: OptionsDataProvider, EarningsRepository       │
│  - Errors: Result[T, Error], AppError                       │
└─────────────────────────────────────────────────────────────┘
```
...
│   │   ├── types.py         # Money, Percentage, OptionChain, etc.
│   │   ├── enums.py         # Recommendation, Quality, Action, etc.
│   │   ├── protocols.py     # Interface definitions
│   │   └── errors.py        # Result types, AppError
│   │
│   ├── config/              # Configuration management
│   │   └── config.py        # Config with from_env(), for_testing()
│   │
│   ├── infrastructure/      # External dependencies
│   │   ├── api/             # API clients
│   │   │   ├── tradier.py          # Tradier options data
│   │   │   └── alpha_vantage.py    # Earnings dates + prices
│   │   ├── database/        # Data access
│   │   │   ├── init_schema.py      # Database initialization
│   │   │   └── repositories/
│   │   │       ├── earnings.py     # Historical moves
│   │   │       ├── metadata.py     # Market cap, sector
│   │   │       └── prices.py       # Daily OHLCV cache
│   │   └── cache/           # Caching layer
│   │       └── memory_cache.py     # TTL cache
│   │
│   ├── application/         # Business logic
│   │   ├── metrics/         # Metric calculators
│   │   │   ├── implied_move.py        # Tier 1
│   │   │   ├── vrp.py                 # Tier 1
│   │   │   ├── consistency.py         # Tier 2
│   │   │   ├── term_structure.py      # Tier 2
│   │   │   ├── skew.py                # Tier 2
│   │   │   ├── execution_quality.py   # Tier 1
│   │   │   ├── market_cap.py          # Tier 3
│   │   │   └── iv_hv_ratio.py         # Tier 3
│   │   └── services/
...
│
├── scripts/
│   ├── analyze.py           # Analyze single ticker
│   ├── scan.py              # Scan multiple tickers
│   └── backfill.py          # Backfill historical data
│
├── data/
│   └── iv_crush_metrics.db  # SQLite database
│
└── logs/
    └── iv_crush.log
```
...
1. **Implied Move** - ATM straddle price / stock price × 100
...
2. **Historical Actual Move** - Past earnings price movements
   - Source: Alpha Vantage API → SQLite cache
...
#
# ✅ AAPL: VRP 2.15x | TRADE (100%)
# ✅ MSFT: VRP 1.85x | TRADE (80%)
# ❌ GOOGL: VRP 1.25x | SKIP (Insufficient edge)
# ✅ NVDA: VRP 2.40x | TRADE (100%)
# ❌ TSLA: VRP 1.65x | SKIP (High inconsistency)
```
...
# ✓ AAPL: 12 earnings backfilled
# ✓ MSFT: 12 earnings backfilled
# ✓ GOOGL: 11 earnings backfilled (1 missing)
...
4. **Implement incrementally** - Follow 21-day timeline with tests
