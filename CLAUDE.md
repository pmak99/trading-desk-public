...
Measures how extreme a ticker's worst historical move is compared to its average. Added after December 2025 when 200-contract MU position caused $134k single-trade loss.
...
| 5.0 Cloud | `gs://trading-desk-data/ivcrush.db` | GCS backups (weekly Sun 3AM) |
...
```bash
gcloud run services update trading-desk --project=trading-desk-prod --region=us-east1 \
...
Base URL: `https://trading-desk-vquzm76kja-ue.a.run.app`
...
# Analyze ticker
curl -s "https://trading-desk-vquzm76kja-ue.a.run.app/api/analyze?ticker=NVDA" -H "X-API-Key: $API_KEY"

# Scan date
curl -s "https://trading-desk-vquzm76kja-ue.a.run.app/api/scan?date=2026-01-15" -H "X-API-Key: $API_KEY"

# Pre-cache sentiment
curl -s -X POST "https://trading-desk-vquzm76kja-ue.a.run.app/prime" -H "X-API-Key: $API_KEY"

# Budget status
curl -s "https://trading-desk-vquzm76kja-ue.a.run.app/api/budget" -H "X-API-Key: $API_KEY"
```
...
- Post-loss improvements embedded (liquidity scoring after $26,930 WDAY/ZS/SYM loss)
