...
```bash
cd $PROJECT_ROOT/2.0 && ./trade.sh scan $(date +%Y-%m-%d) 2>&1 | python ../scripts/export_scan_results.py
```
...
```bash
cd $PROJECT_ROOT/2.0 && ./trade.sh scan $(date +%Y-%m-%d) 2>&1 | tee /tmp/scan_output.txt
```
