...
gcloud run revisions list --service=trading-desk --region=us-east1 --project=trading-desk-prod --format="table(name,status.conditions[0].status,metadata.creationTimestamp)" --limit=3 2>/dev/null
```

#### Step 2: Run 5.0 Tests
```bash
cd "/Users/prashant/PycharmProjects/Trading Desk/5.0" && ../2.0/venv/bin/python -m pytest tests/ -q --tb=short 2>&1 | tail -15
```
...
DEPLOY PLAN:
  ...
  2. Upload to GCS (gs://trading-desk-data/ivcrush.db)
...
Then run:
```bash
cd "/Users/prashant/PycharmProjects/Trading Desk/5.0" && ./deploy.sh 2>&1
```

For `--quick`:
```bash
cd "/Users/prashant/PycharmProjects/Trading Desk/5.0" && ./deploy.sh --quick 2>&1
```

#### Step 5: Post-Deploy Health Check
```bash
SERVICE_URL=$(gcloud run services describe trading-desk --region=us-east1 --project=trading-desk-prod --format="value(status.url)" 2>/dev/null)
...
```bash
gcloud run revisions list --service=trading-desk --region=us-east1 --project=trading-desk-prod --format="table(name,status.conditions[0].status,spec.containers[0].resources.limits.memory,metadata.creationTimestamp)" --limit=3 2>/dev/null
```

### Mode 2: Status Only (`--status`)
```bash
# Current service info
gcloud run services describe trading-desk --region=us-east1 --project=trading-desk-prod --format="yaml(status.url,status.conditions,status.latestReadyRevisionName)" 2>/dev/null

# Recent revisions
gcloud run revisions list --service=trading-desk --region=us-east1 --project=trading-desk-prod --format="table(name,status.conditions[0].status,spec.containers[0].resources.limits.memory,metadata.creationTimestamp)" --limit=5 2>/dev/null
...
### Mode 3: Logs (`--logs`)
```bash
gcloud run services logs read trading-desk --region=us-east1 --project=trading-desk-prod --limit=50 2>/dev/null
```
...
```bash
# List revisions
gcloud run revisions list --service=trading-desk --region=us-east1 --project=trading-desk-prod --format="table(name,status.conditions[0].status,metadata.creationTimestamp)" --limit=5 2>/dev/null
```

Show user the revisions and ask which one to rollback to. Then:
```bash
gcloud run services update-traffic trading-desk --region=us-east1 --project=trading-desk-prod --to-revisions=$REVISION_NAME=100
```
...
PRE-FLIGHT
  [check] gcloud authenticated (project: trading-desk-prod)
...
DEPLOYMENT
  ...
  [check] Uploaded to gs://trading-desk-data/ivcrush.db
...
NEW REVISION
  ...
  Status: ACTIVE
  Memory: 512Mi
  URL: https://trading-desk-vquzm76kja-ue.a.run.app
