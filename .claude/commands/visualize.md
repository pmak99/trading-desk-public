...
```bash
cd /Users/prashant/PycharmProjects/Trading\ Desk && python scripts/visualize_moves.py TICKER [IMPLIED_MOVE]
```
...
```bash
cd /Users/prashant/PycharmProjects/Trading\ Desk/2.0 && sqlite3 -header -csv data/ivcrush.db "SELECT ticker, earnings_date, actual_move_pct, direction FROM historical_moves WHERE ticker = 'TICKER' ORDER BY earnings_date DESC LIMIT 20"
```
...
```bash
cd /Users/prashant/PycharmProjects/Trading\ Desk/2.0 && ./trade.sh scan DATE 2>&1 | grep -E "VRP|EXCELLENT|GOOD|MARGINAL"
```
...
```bash
cd /Users/prashant/PycharmProjects/Trading\ Desk/2.0 && sqlite3 data/ivcrush.db "SELECT ticker, earnings_date, actual_move_pct FROM historical_moves ORDER BY earnings_date DESC LIMIT 50"
```
