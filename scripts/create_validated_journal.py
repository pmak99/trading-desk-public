...
# Repo root, derived from this file's location rather than hardcoded. The
# original had /Users/prashant/... baked in for both input and output, which
# stopped resolving when the checkout moved off macOS.
REPO_ROOT = Path(__file__).resolve().parent.parent
DEFAULT_OUTPUT_DIR = REPO_ROOT / "docs" / "2025 Trades"


def main():
    parser = argparse.ArgumentParser(
        description="Build a validated trading journal from a raw transaction log.",
        epilog=(
            "One-off tooling from the 2025 journal migration; it is not wired into "
            "any skill (its siblings parse_fidelity_csv.py, import_journal_to_db.py, "
            "regroup_strategies.py and parse_trade_statements_v3.py are). The input "
            "used to default to ~/Desktop/transaction_log.txt, so it now has to be "
            "passed explicitly rather than silently reading a file that may not exist."
        ),
    )
    parser.add_argument("input_file", help="Path to the raw transaction log")
    parser.add_argument(
...
if __name__ == "__main__":
    # Propagate main()'s status: it returns 1 when the input file is missing,
    # which otherwise exited 0 and looked like success to any caller.
    sys.exit(main() or 0)
