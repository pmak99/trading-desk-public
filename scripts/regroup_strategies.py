...
def consolidate_fragmented_spreads(db_path: str, dry_run: bool = False, account_type: str = 'TAXABLE') -> int:
    """
    Merge spread strategies that represent the same position but were fragmented
    by the old per-fill pairing logic (N fills per strike → N separate strategies).

    Groups by (symbol, acquired_date, expiration, account_type, strike_set) and
    merges any group with >1 strategy into a single record.
    ...
    cursor.execute("""
        SELECT s.id, s.symbol, s.acquired_date, s.expiration, s.account_type,
               s.gain_loss, s.quantity,
               GROUP_CONCAT(CAST(ROUND(j.strike, 4) AS TEXT)) AS strikes_raw
        FROM strategies s
        JOIN trade_journal j ON j.strategy_id = s.id
        WHERE s.strategy_type = 'SPREAD'
          AND s.account_type = ?
        GROUP BY s.id
        ORDER BY s.id
    """, (account_type,))
    ...
    # Group strategies with the same identity
    groups: Dict[tuple, List[dict]] = defaultdict(list)
    for row in rows:
        strikes = frozenset(row['strikes_raw'].split(',')) if row['strikes_raw'] else frozenset()
        key = (row['symbol'], row['acquired_date'], row['expiration'], row['account_type'], strikes)
    ...
    for (symbol, acq, exp, acct, strikes), strats in fragmented.items():
        combined_pnl = sum(s['gain_loss'] for s in strats)
        print(f"  {symbol} {acq} exp:{exp} strikes:{sorted(strikes)} "
    ...
    try:
        for strats in fragmented.values():
            ...
            cursor.execute(
                f"SELECT COALESCE(SUM(quantity), 0) AS q FROM trade_journal WHERE strategy_id IN ({placeholders})",
                all_ids,
            )
            total_qty = cursor.fetchone()['q']

            cursor.execute(
                "UPDATE strategies SET gain_loss=?, is_winner=?, quantity=? WHERE id=?",
                (combined_pnl, is_winner, total_qty, keep['id']),
            )
