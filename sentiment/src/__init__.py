...
This module imports and re-exports core components to stay in sync with
the proven production system. We import rather than copy to ensure
any core improvements automatically flow to 4.0.
"""
...
# Add core/src to Python path - stay in sync with 2.0
_2_0_src = Path(__file__).parent.parent.parent / "core" / "src"
if _2_0_src.exists():
...
try:
    ...
    logger.warning(f"Could not import core domain modules: {e}")
...
try:
    ...
    logger.warning(f"Could not import core infrastructure modules: {e}")
...
try:
    ...
    logger.warning(f"Could not import core utils modules: {e}")

# sentiment Sentiment-adjusted direction
try:
