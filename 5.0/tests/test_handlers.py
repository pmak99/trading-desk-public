...
def test_sentiment_cache_shares_ivcrush_db():
    """
    Sentiment cache must resolve to the SAME file as DB_PATH (ivcrush.db).

    Fixed 2026-07-27: this test previously asserted the opposite -- that
    SENTIMENT_CACHE_DB_PATH must differ from DB_PATH -- on the premise that
    running as separate processes (5.0 Cloud Run vs 2.0 local) requires
    separate database files. That's wrong: a shared SQLite file is exactly
    how 5.0 already reads/writes historical_moves, earnings_calendar, and
    every other ivcrush.db table (synced via deploy.sh's GCS step), and
    CLAUDE.md documents the sentiment_cache table as migrated INTO
    ivcrush.db (migration 015, May 2026) specifically so both systems share
    one cache. The old assertion enforced the bug it was meant to prevent:
    5.0 silently pointed at either the archived 4.0/data/sentiment_cache.db
    or an empty per-instance Cloud Run file, and never shared a single
    cached sentiment entry with 2.0 in either direction.
    """
