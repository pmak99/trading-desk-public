...
def test_liquidity_excellent():
    """EXCELLENT: OI >= 5x, spread <= 12%."""
    tier = classify_liquidity_tier(oi=1000, spread_pct=10.0, position_size=100)
    assert tier == "EXCELLENT"

def test_liquidity_good():
    """GOOD: OI 2-5x, spread 12-18%."""
    tier = classify_liquidity_tier(oi=300, spread_pct=15.0, position_size=100)
    assert tier == "GOOD"

def test_liquidity_warning():
    """WARNING: OI 1-2x, spread 18-25%."""
    tier = classify_liquidity_tier(oi=150, spread_pct=20.0, position_size=100)
    assert tier == "WARNING"
...
def test_liquidity_reject_wide_spread():
    """REJECT: spread > 25%."""
    tier = classify_liquidity_tier(oi=1000, spread_pct=30.0, position_size=100)
    assert tier == "REJECT"
