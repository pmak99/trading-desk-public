...
# --- Tests for calculate_implied_move_from_chain ---

def test_calculate_implied_move_from_chain_basic():
    """Straightforward chain with populated bid/ask."""
    chain = [
        {"strike": 100.0, "option_type": "call", "bid": 5.0, "ask": 5.5},
        {"strike": 100.0, "option_type": "put", "bid": 4.0, "ask": 4.5},
    ]
    result = calculate_implied_move_from_chain(chain, stock_price=100.0)
    assert result is not None
    # call_mid = 5.25, put_mid = 4.25, straddle = 9.5, move = 9.5%
    assert result["implied_move_pct"] == pytest.approx(9.5, rel=0.01)


def test_calculate_implied_move_from_chain_null_bid_does_not_crash():
    """
    Regression test (2026-07-27 production incident): Tradier returns
    explicit `"bid": null` on thin/closed-market chains. `.get(key, 0)`
    only applies its default when the key is MISSING, not when the value
    is None -- so a naive `call.get("bid", 0) + call.get("ask", 0)` threw
    TypeError: unsupported operand type(s) for +: 'NoneType' and
    'NoneType' in production (PYPL, V; reproduced right after market
    close). Null bid/ask must degrade to a 0 mid price, which the
    call_mid<=0 guard then correctly turns into a None result (fallback
    to the estimate path), not a crash.
    """
    chain = [
        {"strike": 100.0, "option_type": "call", "bid": None, "ask": None},
        {"strike": 100.0, "option_type": "put", "bid": 4.0, "ask": 4.5},
    ]
    result = calculate_implied_move_from_chain(chain, stock_price=100.0)
    assert result is None  # call_mid == 0 -> triggers fallback, not a crash


def test_calculate_implied_move_from_chain_missing_bid_key_does_not_crash():
    """Missing bid/ask keys entirely (not just null) also degrade safely."""
    chain = [
        {"strike": 100.0, "option_type": "call", "ask": 5.5},
        {"strike": 100.0, "option_type": "put", "bid": 4.0, "ask": 4.5},
    ]
    result = calculate_implied_move_from_chain(chain, stock_price=100.0)
    assert result is not None
    # call_mid = (0 + 5.5) / 2 = 2.75, put_mid = 4.25
    assert result["implied_move_pct"] == pytest.approx(7.0, rel=0.01)


def test_calculate_implied_move_from_chain_missing_put_side():
    """Chain has a call at the closest strike but no matching put -> None."""
    chain = [
        {"strike": 50.0, "option_type": "call", "bid": 1.0, "ask": 1.5},
    ]
    result = calculate_implied_move_from_chain(chain, stock_price=100.0)
