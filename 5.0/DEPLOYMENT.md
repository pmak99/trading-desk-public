...
# Find opportunities
curl -H "X-API-Key: $API_KEY" https://trading-desk-670614791512.us-east1.run.app/api/whisper

# Scan specific date for earnings
curl -H "X-API-Key: $API_KEY" "https://trading-desk-670614791512.us-east1.run.app/api/scan?date=2026-01-15"

# Pre-cache sentiment for upcoming earnings
curl -X POST -H "X-API-Key: $API_KEY" https://trading-desk-670614791512.us-east1.run.app/prime

# Get detailed budget status
curl -H "X-API-Key: $API_KEY" https://trading-desk-670614791512.us-east1.run.app/api/budget
```
...
# Create new project
gcloud projects create trading-desk-prod --name="Trading Desk Production"

# Set as default
gcloud config set project trading-desk-prod
...
gcloud run deploy trading-desk \
  ...
  --set-env-vars GOOGLE_CLOUD_PROJECT=trading-desk-prod
```
...
gcloud run services add-iam-policy-binding trading-desk \
  --region us-east1 \
  --member="serviceAccount:trading-desk-scheduler@trading-desk-prod.iam.gserviceaccount.com" \
...
gcloud scheduler jobs create http trading-desk-dispatch \
  ...
  --http-method=POST \
  --oidc-service-account-email=trading-desk-scheduler@trading-desk-prod.iam.gserviceaccount.com
...
```bash
cd "/Users/prashant/PycharmProjects/Trading Desk/5.0/terraform"
...
```bash
SERVICE_URL="https://trading-desk-670614791512.us-east1.run.app"
