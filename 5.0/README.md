...
```bash
curl -H "X-API-Key: $API_KEY" "https://trading-desk-vquzm76kja-ue.a.run.app/api/health"
```
...
gcloud run deploy trading-desk \
  ...
  --set-env-vars 'GOOGLE_CLOUD_PROJECT=trading-desk-prod,GCS_BUCKET=trading-desk-data'

# IMPORTANT: Run calendar-sync after deployment (database is ephemeral)
curl -X POST "https://trading-desk-670614791512.us-east1.run.app/dispatch?force=calendar-sync" \
  -H "X-API-Key: $API_KEY"
```
