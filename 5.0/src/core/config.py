...
class Settings:
    ...
    def SENTIMENT_CACHE_DB_PATH(self) -> str:
        ...
        The `sentiment_cache` table lives IN ivcrush.db (migrated there from
        the standalone 4.0/data/sentiment_cache.db in schema migration 015,
        May 2026 -- CLAUDE.md: "All skill commands read/write ivcrush.db").
        This must resolve to the SAME file as DB_PATH, not a separate one:
        fixed 2026-07-27 -- this previously pointed at either the archived
        4.0 db (locally) or an empty per-instance Cloud Run file, so 5.0
        never actually shared a cached sentiment entry with 2.0 in either
        direction despite both reading/writing a table of the same name.
        Kept as its own property (rather than inlining settings.DB_PATH at
        call sites) so a future split back into a separate file only needs
        one line changed. SENTIMENT_CACHE_DB_PATH env var override kept for
        tests that need an isolated scratch file.
        """
