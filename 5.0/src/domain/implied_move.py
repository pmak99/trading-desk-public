...
def calculate_implied_move_from_chain(
    ...
    # Use mid prices. `.get(key, 0)` only defaults on a MISSING key -- Tradier
    # returns explicit `"bid": null` on thin/closed-market chains, and `.get()`
    # then returns None (not the default), so plain addition threw
    # "unsupported operand type(s) for +: 'NoneType' and 'NoneType'" in
    # production (2026-07-27, reproduced on PYPL/V right after market close).
    # `or 0` catches both missing and explicit-null; matches the established
    # pattern already used for the same hazard in analyze.py/scan.py.
    call_mid = ((call.get("bid") or 0) + (call.get("ask") or 0)) / 2
    put_mid = ((put.get("bid") or 0) + (put.get("ask") or 0)) / 2
