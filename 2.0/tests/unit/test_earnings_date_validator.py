...
class TestEarningsDateValidator:
    ...
    def test_known_finnhub_date_skips_live_finnhub_call(
        self, validator, mock_yahoo_finance, mock_finnhub
    ):
        """A caller that already has a bulk-fetched Finnhub date (e.g. the
        calendar sync's 3-month bulk pull) should be able to pass it in and
        skip a redundant live per-ticker Finnhub call — making one of those
        per ticker in a multi-ticker run was what exhausted Finnhub's rate
        limit mid-run (root-caused 2026-08-24)."""
        ticker = "TGT"
        yf_date = date(2025, 12, 8)
        fh_date = date(2025, 12, 8)
        timing = EarningsTiming.AMC
        ...
        result = validator.validate_earnings_date(
            ticker, known_finnhub_date=(fh_date, timing)
        )
        ...
        assert not validation.has_conflict
        assert validation.consensus_date == yf_date
        mock_finnhub.get_earnings_calendar.assert_not_called()

    def test_finnhub_rate_limited_not_treated_as_consensus(
        self, validator, mock_yahoo_finance, mock_finnhub
    ):
        """A Finnhub rate-limit/transient failure must not be silently accepted
        as single-source consensus — that would let a lone Yahoo date pass
        as if two sources had corroborated it. Should flag has_conflict so
        callers route it to NEEDS REVIEW instead of writing it."""
        ticker = "TGT"
        earnings_date = date(2025, 12, 8)
        timing = EarningsTiming.AMC
        ...
        mock_finnhub.get_earnings_calendar.return_value = Result.Err(
            AppError(ErrorCode.RATELIMIT, "Finnhub rate limit exceeded")
        )
        ...
        assert "Finnhub fetch failed" in validation.conflict_details
