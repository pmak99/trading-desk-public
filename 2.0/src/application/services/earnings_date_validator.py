...
class EarningsDateValidator:
    ...
    def validate_earnings_date(
        self, ticker: str,
        known_finnhub_date: Optional[Tuple[date, EarningsTiming]] = None,
        ...
        Args:
            ...
            known_finnhub_date: Optional (date, timing) already obtained from a
                bulk Finnhub fetch the caller made moments earlier (e.g. the
                calendar sync's 3-month bulk pull). When provided, skips a
                redundant live per-ticker Finnhub call — callers that iterate
                many tickers per run (calendar sync) would otherwise multiply
                their Finnhub call count by ~1 per ticker on top of the bulk
                fetch, which is what exhausted Finnhub's rate limit mid-run
                and forced later tickers into single-source Yahoo-only
                "consensus" (see the finnhub_unavailable handling below).
                Found and root-caused 2026-08-24.
        ...
        # Fetch from Finnhub — reuse a caller-supplied bulk result if given,
        # otherwise make a live per-ticker call.
        finnhub_unavailable = False
        if known_finnhub_date is not None:
            earnings_date, timing = known_finnhub_date
            sources.append(EarningsDateInfo(
            ...
            logger.debug(f"{ticker}: Finnhub (reused bulk fetch) = {earnings_date} ({timing.value})")
        elif self.finnhub:
            ...
            elif av_result.is_err and av_result.error.code != ErrorCode.NODATA:
                # Finnhub was configured and attempted but failed transiently
                # (rate limit, timeout, network error) rather than genuinely
                # having no data for this ticker. That's not the same as
                # "Finnhub agrees" or "Finnhub has no opinion" — it means we
                # never actually got Finnhub's answer, so a lone Yahoo result
                # below must not be accepted as cross-source consensus.
                finnhub_unavailable = True
                logger.warning(
                    f"{ticker}: Finnhub fetch failed ({av_result.error.code.value}) "
                    f"— cannot corroborate, will not treat remaining source as consensus"
                )
        ...
        elif finnhub_unavailable:
            has_conflict = True
            conflict_details = (
                f"Finnhub fetch failed — only {sources[0].source.value} responded "
                f"({sources[0].earnings_date}); no second source to corroborate"
            )
