...
def check_liquidity_with_tier(ticker: str, expiration: date, container: Container) -> Tuple[bool, str]:
    ...
    try:
        # Get option chain (cached — dedupes against other fetches of the
        # same (ticker, expiration) chain within a scan)
        tradier = container.cached_options_provider
...
def check_liquidity_hybrid(
    ...
    try:
        # Get option chain (cached — implied move / skew curve fetch the same
        # (ticker, expiration) chain earlier in the same ticker's analysis)
        tradier = container.cached_options_provider
        chain_result = tradier.get_option_chain(ticker, expiration)
