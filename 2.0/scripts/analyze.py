...
Notes:
    ...
    try:
        ...
        # "One position per ticker per earnings event" (CLAUDE.md) — a prior
        # journaled position on this exact event already exists. Loud and
        # ahead of the recommendation: a real repair campaign on record
        # turned a large first loss into a catastrophic one — the repair
        # itself, not the original loss, was the failure mode.
        if analysis.existing_position_warning:
            logger.info("\n" + "=" * 80)
            logger.info("🚨 EXISTING POSITION ALREADY ON THIS EARNINGS EVENT")
            logger.info("=" * 80)
            logger.info(f"   {analysis.existing_position_warning}")
            logger.info("   CLAUDE.md: \"One position per ticker per earnings event.")
            logger.info("   No second bet, no repair attempt.\" REPAIR win rate: 20%.")
            logger.info("   (Best-effort check against journaled /journal history —")
            logger.info("   verify your actual open positions before trading.)")
