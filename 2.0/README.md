...
| `./trade.sh sync-cloud` | Sync local DB with GCS + Google Drive backup (30-day retention) |
...
│   ├── data_sources/           Historical price sources, Yahoo Finance earnings dates
│   ├── database/               SQLite layer
│   │   ├── connection_pool.py  Thread-safe connection pooling
│   │   ├── migrations/         MigrationManager (schema v19, runs on startup)
│   │   └── repositories/       Repository pattern (analysis, earnings, prices)
│   ├── cache/                  In-memory LRU + DB hybrid cache
│   └── monitoring/             Runtime monitoring
├── utils/                      concurrent_scanner, rate_limiter (token bucket, waits ≤30s at Tradier call sites), market_hours
...
- ORATS disabled (current state): `compute_tradier_r_slp30_proxy(slope_atm)` substitutes at confidence 0.3 — direction preserved, magnitude softened. Proxy: `RSLP30_MEAN + slope_atm × 0.0027`, clamped ±2σ.
- The fused result is **not persisted** — `bias_predictions` stores Tradier-only skew; the fused value is computed fresh each run.
- This proxy is single-source (fused with the Tradier signal it derives from) — the compound-risk crush-rate calibration came from a prior two-source setup and may not transfer cleanly; treat proxy-path fused skew as lower-confidence.

## Term Structure & Calendar Pilot
...
| `backfill_historical.py` | Backfill historical moves for a ticker from Twelve Data (signed open→close `intraday_move_pct`); `expand_universe.py` is dead post-ORATS, this is the only path for new tickers |
| `sync_earnings_calendar.py` | Refresh upcoming earnings — Finnhub bulk fetch (chunked, since Finnhub caps bulk responses at 1,500 entries), cross-validated against Yahoo Finance; a write that would change an existing confirmed date requires both sources to agree or it's skipped and flagged for review, never applied on one source's say-so |
| `cleanup_duplicate_moves.py` | Dedupe adjacent-date rows in `historical_moves`/`earnings_calendar` — rerun if a future backfill or sync-cloud merge reintroduces pairs |
...
| `scan.py` | Scan pipeline entry point (called by `trade.sh`) |

## Database

`data/ivcrush.db` | Schema v19 | migrations run automatically on startup via `MigrationManager` (`src/infrastructure/database/migrations/`)
...
`trade.sh sync-cloud` creates a timestamped local backup in `backups/` and a separate 30-day-retention backup to Google Drive, then pushes to `gs://your-gcs-bucket/ivcrush.db`.
...
FINNHUB_API_KEY=xxx       # Earnings calendar (primary source)
...
# Dead — not read anywhere in current code (superseded by Finnhub + Yahoo Finance;
# grep confirms zero live references outside one historical regression test)
...
```bash
./venv/bin/python -m pytest tests/ -v           # 1,373 tests
